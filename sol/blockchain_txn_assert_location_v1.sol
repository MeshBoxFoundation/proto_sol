...
library SmartmeshBlockchain_txn_assert_location_v1 {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 4) {
        pointer += _read_gateway_signature(pointer, bs, r);
      } else
      if (fieldId == 5) {
   ...
   * @return The number of bytes decoded
   */
  function _read_gateway_signature(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (bytes memory x, uint256 sz) = ProtoBufRuntime._decode_bytes(p, bs);
    r.gateway_signature = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    if (r.gateway_signature.length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      4,
      ProtoBufRuntime.WireType.LengthDelim,
      pointer,
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.gateway_signature, pointer, bs);
    }
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(r.gateway_signature.length);
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.gateway_signature = input.gateway_signature;
