...
library SmartmeshBlockchain_txn_create_htlc_v1 {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 4) {
        pointer += _read_hashlock(pointer, bs, r);
      } else
      if (fieldId == 5) {
        pointer += _read_timelock(pointer, bs, r);
      } else
      if (fieldId == 6) {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.hashlock, pointer, bs);
    }
    if (r.timelock != 0) {
    pointer += ProtoBufRuntime._encode_key(
      5,
    ...
    pointer += ProtoBufRuntime._encode_uint64(r.timelock, pointer, bs);
    }
    if (r.amount != 0) {
    pointer += ProtoBufRuntime._encode_key(
      6,
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(r.hashlock.length);
    e += 1 + ProtoBufRuntime._sz_uint64(r.timelock);
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.hashlock = input.hashlock;
    output.timelock = input.timelock;
