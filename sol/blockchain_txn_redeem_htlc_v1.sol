...
library SmartmeshBlockchain_txn_redeem_htlc_v1 {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 3) {
        pointer += _read_preimage(pointer, bs, r);
      } else
      if (fieldId == 4) {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.preimage, pointer, bs);
    }
    if (r.fee != 0) {
    pointer += ProtoBufRuntime._encode_key(
      4,
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(r.preimage.length);
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.preimage = input.preimage;
