...
library SmartmeshBlockchain_txn_unstake_validator_v1 {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 6) {
        pointer += _read_stake_release_height(pointer, bs, r);
      } else
      {
   ...
   * @return The number of bytes decoded
   */
  function _read_stake_release_height(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (uint64 x, uint256 sz) = ProtoBufRuntime._decode_uint64(p, bs);
    r.stake_release_height = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_uint64(r.stake_release_height, pointer, bs);
    }
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_uint64(r.stake_release_height);
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.stake_release_height = input.stake_release_height;
