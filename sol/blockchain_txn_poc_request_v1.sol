...
library SmartmeshBlockchain_txn_poc_request_v1 {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 2) {
        pointer += _read_secret_hash(pointer, bs, r);
      } else
      if (fieldId == 3) {
        pointer += _read_onion_key_hash(pointer, bs, r);
      } else
      if (fieldId == 4) {
        pointer += _read_block_hash(pointer, bs, r);
      } else
      if (fieldId == 5) {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.secret_hash, pointer, bs);
    }
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.block_hash, pointer, bs);
    }
    if (r.fee != 0) {
    pointer += ProtoBufRuntime._encode_key(
      5,
    ...
    pointer += ProtoBufRuntime._encode_uint32(r.version, pointer, bs);
    }
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(r.secret_hash.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(r.onion_key_hash.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(r.block_hash.length);
    e += 1 + ProtoBufRuntime._sz_uint64(r.fee);
    e += 1 + ProtoBufRuntime._sz_lendelim(r.signature.length);
    e += 1 + ProtoBufRuntime._sz_uint32(r.version);
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.secret_hash = input.secret_hash;
    output.onion_key_hash = input.onion_key_hash;
    output.block_hash = input.block_hash;
