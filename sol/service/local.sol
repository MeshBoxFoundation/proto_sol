...
library SmartmeshLocalSign_req {
...
library SmartmeshLocalPubkey_res {
...
library SmartmeshLocalPubkey_req {
...
library SmartmeshLocalConfig_value {
...
library SmartmeshLocalConfig_res {


  //struct definition
  struct Data {
    SmartmeshLocalConfig_value.Data[] values;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_values(pointer, bs, nil(), counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    pointer = offset;
    if (counters[1] > 0) {
      require(r.values.length == 0);
      r.values = new SmartmeshLocalConfig_value.Data[](counters[1]);
    }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_values(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_unpacked_repeated_values(
    uint256 p,
    bytes memory bs,
    Data memory r,
    uint[2] memory counters
     ...
     */
    (SmartmeshLocalConfig_value.Data memory x, uint256 sz) = _decode_SmartmeshLocalConfig_value(p, bs);
    if (isNil(r)) {
      counters[1] += 1;
    } else {
      r.values[r.values.length - counters[1]] = x;
      counters[1] -= 1;
    }
    return sz;
  }
   ...
   */
  function _decode_SmartmeshLocalConfig_value(uint256 p, bytes memory bs)
    internal
    pure
    returns (SmartmeshLocalConfig_value.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (SmartmeshLocalConfig_value.Data memory r, ) = SmartmeshLocalConfig_value._decode(pointer, bs, sz);
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.values.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += SmartmeshLocalConfig_value._encode_nested(r.values[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.values.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(SmartmeshLocalConfig_value._estimate(r.values[i]));
    }
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    for(uint256 i1 = 0; i1 < input.values.length; i1++) {
      output.values.push(input.values[i1]);
    }
  ...
  //array helpers for Values
  /**
   * @dev Add value to an array
   * @param self The in-memory struct
   * @param value The value to add
   */
  function addValues(Data memory self, SmartmeshLocalConfig_value.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    SmartmeshLocalConfig_value.Data[] memory tmp = new SmartmeshLocalConfig_value.Data[](self.values.length + 1);
    for (uint256 i = 0; i < self.values.length; i++) {
      tmp[i] = self.values[i];
    }
    tmp[self.values.length] = value;
    self.values = tmp;
  }
...
library SmartmeshLocalConfig_req {
...
library SmartmeshLocalHeight_req {
...
library SmartmeshLocalKeyed_uri {
...
library SmartmeshLocalEcdh_res {
...
library SmartmeshLocalHeight_res {
  ...
  struct Data {
    uint64 height;
    uint64 block_age;
    SmartmeshLocalKeyed_uri.Data gateway;
  }
  ...
  function _read_gateway(
    ...
    (SmartmeshLocalKeyed_uri.Data memory x, uint256 sz) = _decode_SmartmeshLocalKeyed_uri(p, bs);
    r.gateway = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_SmartmeshLocalKeyed_uri(uint256 p, bytes memory bs)
    internal
    pure
    returns (SmartmeshLocalKeyed_uri.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (SmartmeshLocalKeyed_uri.Data memory r, ) = SmartmeshLocalKeyed_uri._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += SmartmeshLocalKeyed_uri._encode_nested(r.gateway, pointer, bs);
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(SmartmeshLocalKeyed_uri._estimate(r.gateway));
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    SmartmeshLocalKeyed_uri.store(input.gateway, output.gateway);
...
library SmartmeshLocalEcdh_req {
...
library SmartmeshLocalSign_res {
