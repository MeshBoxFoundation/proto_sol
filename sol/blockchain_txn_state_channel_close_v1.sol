...
library SmartmeshBlockchain_txn_state_channel_close_v1 {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 2) {
        pointer += _read_closer(pointer, bs, r);
      } else
      if (fieldId == 3) {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.closer, pointer, bs);
    }
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(r.closer.length);
