...
library SmartmeshBlockchain_gossip_block {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_from(pointer, bs, r);
      } else
      if (fieldId == 2) {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.from, pointer, bs);
    }
    if (r.block.length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      2,
    ...
    pointer += ProtoBufRuntime._encode_bytes(r.block, pointer, bs);
    }
    if (r.hash.length != 0) {
    pointer += ProtoBufRuntime._encode_key(
      3,
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(r.from.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(r.block.length);
