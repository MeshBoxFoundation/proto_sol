...
library SmartmeshBlockchain_txn_reward_v2 {
...
library SmartmeshBlockchain_txn_rewards_v2 {
  ...
  struct Data {
    uint64 start_epoch;
    uint64 end_epoch;
    SmartmeshBlockchain_txn_reward_v2.Data[] rewards;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    if (counters[3] > 0) {
      ...
      r.rewards = new SmartmeshBlockchain_txn_reward_v2.Data[](counters[3]);
    }
  ...
  function _read_unpacked_repeated_rewards(
     ...
     */
    (SmartmeshBlockchain_txn_reward_v2.Data memory x, uint256 sz) = _decode_SmartmeshBlockchain_txn_reward_v2(p, bs);
    if (isNil(r)) {
      counters[3] += 1;
    } else {
   ...
   */
  function _decode_SmartmeshBlockchain_txn_reward_v2(uint256 p, bytes memory bs)
    internal
    pure
    returns (SmartmeshBlockchain_txn_reward_v2.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (SmartmeshBlockchain_txn_reward_v2.Data memory r, ) = SmartmeshBlockchain_txn_reward_v2._decode(pointer, bs, sz);
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.rewards.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += SmartmeshBlockchain_txn_reward_v2._encode_nested(r.rewards[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    for(i = 0; i < r.rewards.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(SmartmeshBlockchain_txn_reward_v2._estimate(r.rewards[i]));
    }
    return e;
  }
  // empty checker
   ...
   */
  function addRewards(Data memory self, SmartmeshBlockchain_txn_reward_v2.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    SmartmeshBlockchain_txn_reward_v2.Data[] memory tmp = new SmartmeshBlockchain_txn_reward_v2.Data[](self.rewards.length + 1);
